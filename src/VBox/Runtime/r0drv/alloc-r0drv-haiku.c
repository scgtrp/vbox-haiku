/* $Id: alloc-r0drv-haiku.cpp 32708 2010-09-23 11:18:51Z vboxsync $ */
/** @file
...
int rtR0MemAllocEx(size_t cb, uint32_t fFlags, PRTMEMHDR *ppHdr)
{
    ...
    if (RT_UNLIKELY(!pHdr))
    {
        dprintf("rtR0MemAllocEx(%#zx, %#x) failed\n", cb + sizeof(*pHdr), fFlags);
...
RTR0DECL(void *) RTMemContAlloc(PRTCCPHYS pPhys, size_t cb) RT_NO_THROW
{
    ...
    if (area >= 0)
    {
        ...
        AssertMsgFailed(("Cannot get_memory_map for contig alloc! cb=%#zx\n", cb));
        delete_area(area);
    } else
    	AssertMsgFailed(("Cannot create_area for contig alloc! cb=%#zx error=0x%08lx\n", cb, area));
    return NULL;
}
...
RTR0DECL(void) RTMemContFree(void *pv, size_t cb) RT_NO_THROW
{
    ...
    if (pv)
    {
        ...
        if (area < B_OK)
        	AssertMsgFailed(("Cannot find area to delete! cb=%#zx error=0x%08lx\n", cb, area));
        else
        	delete_area(area);
    }
}
