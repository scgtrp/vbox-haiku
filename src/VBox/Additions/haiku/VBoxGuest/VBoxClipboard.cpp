    ...
    if (RT_SUCCESS (rc)) {
		...
		if (err >= B_OK) {
			...
			if (err == B_OK)
		        ...
		        LogRel(("VBoxClipboardService: Error watching the system clipboard: 0x%08lx\n",
		        	err));
		} else
	        LogRel(("VBoxClipboardService: Error starting service thread: 0x%08lx\n",
    	    	err));
    ...
    for (;;)
    {
        ...
        if (RT_SUCCESS(rc)) {
            switch (u32Msg) {
                ...
                default:
                    Log(("VBoxClipboardService::%s: Unsupported message from host! Message = %ld\n", __FUNCTION__, u32Msg));
            }
        } else
        	fExiting = true;
